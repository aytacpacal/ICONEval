...
def make_portable_html(
    html_path: Path,
    rewrite_html_links: bool = False,
) -> Path:
    """Create a copy of an HTML file with all local images embedded as base64.

    Reads ``html_path``, replaces every ``src`` attribute that points to a
    local image file with an inline ``data:`` URI, and writes the result to a
    new file whose name has ``_portable`` appended before the ``.html``
    extension.

    Parameters
    ----------
    html_path:
        Path to an existing HTML file whose sibling image files should be
        embedded.
    rewrite_html_links:
        If ``True``, rewrite local HTML ``href`` links and ``window.open``
        calls so that ``foo/index.html`` becomes ``foo/index_portable.html``
        and ``index_realm.html`` becomes ``index_realm_portable.html``.  Use
        this when creating portable copies of the top-level realm index files
        so that their navigation links point to other portable files.

    Returns
    -------
    Path
        Path to the newly created portable HTML file.
    ...
    if rewrite_html_links:
        ...
        def _rewrite_href(match: re.Match) -> str:
            ...
            if href.startswith(("http://", "https://", "#", "data:")):
                return match.group(0)
            # recipe_*/index.html  →  recipe_*/index_portable.html
            if href.endswith("/index.html"):
                href = href[: -len("index.html")] + "index_portable.html"
            # bare index*.html (realm navigation)  →  index*_portable.html
            elif re.fullmatch(r"index(?:_[^/\"']+)?\.html", href):
                stem = href[: -len(".html")]
                href = f"{stem}_portable.html"
        ...
        # Rewrite window.open('index*.html', ...) in realm navigation buttons
        def _rewrite_window_open(match: re.Match) -> str:
            quote = match.group(1)
            name = match.group(2)
            rest = match.group(3)
            if re.fullmatch(r"index(?:_[^'\"]+)?\.html", name):
                stem = name[: -len(".html")]
                name = f"{stem}_portable.html"
            return f"window.open({quote}{name}{quote}{rest}"

        html = re.sub(
            r"window\.open\((['\"])(index(?:_[^'\"]+)?\.html)\1([^)]*)\)",
            _rewrite_window_open,
            html,
        )

    portable_path = html_path.with_name(f"{html_path.stem}_portable.html")
    portable_path.write_text(html)
    return portable_path


def summarize_portable(esmvaltool_output_dir: Path) -> None:
    """Create portable (self-contained) versions of all summary HTML files.

    Reads the HTML files previously written by :func:`summarize` and produces
    ``*_portable.html`` copies in which every local image is embedded as a
    base64 ``data:`` URI, so the file can be shared as a single file.

    For each recipe subdirectory a portable ``index_portable.html`` is also
    created so that the links from the top-level index work without access to
    ...
    """
    # First, create portable versions of individual recipe index pages so
    # that links from the realm index portable files resolve correctly.
    for recipe_dir in sorted(esmvaltool_output_dir.glob("recipe_*")):
        recipe_index = recipe_dir / "index.html"
        if recipe_index.is_file():
            portable_recipe = make_portable_html(recipe_index)
            logger.debug(f"Wrote file://{portable_recipe.absolute()}")

    # Then create realm-level portable index files with rewritten HTML links.
    realms: list[RealmType] = [
        "all",
        "atmosphere",
        "ocean",
    ...
    for realm in realms:
        html_path = esmvaltool_output_dir / _get_index_html_name(realm)
        if not html_path.is_file():
            continue
        portable_path = make_portable_html(html_path, rewrite_html_links=True)
        logger.debug(f"Wrote file://{portable_path.absolute()}")

    portable_index = esmvaltool_output_dir / "index_portable.html"
    logger.info(
        f"Successfully created portable HTML "
        f"<cyan>{portable_index}</cyan>",
    )
