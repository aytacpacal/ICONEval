...
class RecipeTemplate(Template):
    ...
    def get_recipe(
        ...
        **extra_facets: FacetType,
        ...
        # Determine default project/dataset from first simulation
        ...
        placeholders: dict[str, FacetType] = {
            "{{dataset}}": extra_facets.get("dataset", default_dataset),
            "{{dataset_list}}": dataset_list,
            "{{project}}": extra_facets.get("project", default_project),
            "{{timerange}}": timerange,
        }
...
class ESMValToolConfigTemplate(Template):
    ...
    def _fill_projects(
        ...
        This method now works generically with any model. If a simulation
        has a model_config attached, it uses that to build the project
        configuration. Otherwise, it falls back to ICON/EMAC defaults
        for backward compatibility.
        """
        config_yaml = deepcopy(config_yaml)
        projects: dict[str, dict[str, Any]] = config_yaml.get("projects", {})

        # Group simulations by project name
        project_data: dict[str, dict[str, Any]] = {}

        for simulation_info in simulations_info:
            mc = simulation_info.model_config

            if mc is not None:
                # Use model config to build project data
                project_name = mc.project
                if project_name not in project_data:
                    project_data[project_name] = {}

                sources = mc.build_data_source_for_exp(
                ...
                if "ICON" not in project_data:
                    project_data["ICON"] = {}
                sources = icon_mc.build_data_source_for_exp(
                    simulation_info.exp, simulation_info.path
                )
                for source in sources:
        ...
        for project_name, data_config in project_data.items():
            projects[project_name] = {"data": data_config}
